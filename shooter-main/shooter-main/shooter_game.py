...
player = pygame.Rect(10, 400, 70, 100)  # Встановлено y = 400
...
galaxy = pygame.transform.scale(galaxy,(500,500))
...
while game:
    ...
    # Рух тільки вправо і вліво
    if keys[pygame.K_a]:
        player.x -= 5
    if keys[pygame.K_d]:
        player.x += 5

    # Обмеження руху гравця в межах екрана
    if player.x < 0:
        player.x = 0
    if player.x > 500 - player.width:
        player.x = 500 - player.width

    # Залишити y на постійній координаті
    player.y = 400
